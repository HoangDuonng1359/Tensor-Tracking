...
class PathConfig:
  ...
  FCCA_DIR: Path = DATA_PROCESSED / "05_fcca_results"
  OUTPUTS_DIR: Path = BASE_PATH / "outputs" / "eda_1024"

  def __post_init__(self):
    """Create directories if they do not exist."""
    for d in [self.EPOCHS_DIR, self.REFINED_DIR, self.TENSOR_DIR, self.TRACKING_DIR, self.FCCA_DIR, self.OUTPUTS_DIR]:
