...
def main():
    ...
    for condition in ["incorrect", "correct"]:
        ...
        for lr_name, X_clean in [("HOSVD", X_hosvd), ("HO-RLSL", X_ho_rlsl)]:
            print(f"  Precomputing subject labels for {lr_name} in parallel...")
            ...
            for s in range(n_subjects):
                # Only pass the data for the specific subject to minimize IPC overhead
                tasks.append((s, X_clean[s].copy(), n_times, n_nodes))

            # Run in parallel pool
            # Use os.cpu_count() - 1 or default, capped at 4 to prevent MemoryError
            max_workers = min(4, max(1, (os.cpu_count() or 4) - 1))
            print(f"    Spawning ProcessPoolExecutor with {max_workers} workers...")
            with ProcessPoolExecutor(max_workers=max_workers) as executor:
                results = list(executor.map(process_single_subject, tasks))

            # Re-collect results
            for s, labels_s, scores_s, method_s in results:
