...
# Low-rank dau vao: luon lay tu file nay.
DATA_FILE_OVERRIDE: str | None = "../../outputs/ho_rlsl_v2_results.npz"

# Lay changepoint 1024Hz ERN tu 5 phuong phap trong final_results.
CHANGEPOINT_KEY = "1024Hz_ERN"
CHANGEPOINT_METHOD_FILES = {
    "CP_Tracking": Path("final_results") / "CP_Tracking" / "CP_ChangePoints.npy",
    "DMD": Path("final_results") / "DMD" / "DMD_ChangePoints.npy",
    "HOSVD": Path("final_results") / "HOSVD" / "HOSVD_ChangePoints.npy",
    "HO_RLSL": Path("final_results") / "HO_RLSL" / "HORLSL_ChangePoints.npy",
    "PELT": Path("final_results") / "PELT" / "PELT_ChangePoints.npy",
}

OUTPUT_DIR_NAME = "../../outputs/FCCA_ho_rlsl_results_1024Hz_ERN_by_changepoint"
...
def cluster_ern_fcca_paper_legacy_single_method(k_input: Any = None) -> dict[str, Any]:
    """
    ...
    if DATA_FILE_OVERRIDE:
        data_file = Path(DATA_FILE_OVERRIDE)
    ...
    raw_bids_dir = first_existing_dir(
        [
            ...
            base_dir.parent.parent / "data" / "ERN Raw Data BIDS-Compatible",
            base_dir.parent.parent / "data" / "ERN_Raw_Data_BIDS-Compatible",
            Path.cwd() / "ERN Raw Data BIDS-Compatible",
            Path.cwd() / "ERN_Raw_Data_BIDS-Compatible",
        ]
    )
    ...
    change_points = choose_change_points_from_metadata(
        metadata, n_frames, times_ms, cfg.analysisWindowMs
    )
...
def load_npz_tensor(filename: Path) -> tuple[np.ndarray, dict[str, Any]]:
    with np.load(filename, allow_pickle=True) as loaded:
        if "low_rank" not in loaded.files:
            raise ValueError(f"Cannot find low_rank.npy inside {filename}.")
        tensor = np.asarray(loaded["low_rank"], dtype=np.float64)
        metadata = {
...
def make_interval_specs(
    ...
    c1 = int(change_points[0])
    c2 = int(change_points[1])
...
def cluster_ern_fcca_paper(k_input: Any = None) -> dict[str, Any]:
    """
    Final pipeline: use low_rank from ho_rlsl_results.npz and run FCCA once
    for each 1024Hz ERN change-point method found in final_results.
    """
    if k_input is None:
        k_input = []
    ...
    method_files = resolve_changepoint_method_files(base_dir)
    ...
    output_root = base_dir / "final_results" / OUTPUT_DIR_NAME
    output_root.mkdir(parents=True, exist_ok=True)

    cfg = Config(requestedK=k_input)
    np.random.seed(cfg.randomSeed)

    print("=== FCCA final: ho_rlsl_results low_rank + final_results changepoints ===")
    print(f"Data file: {data_file}")
    print(f"Change-point key: {CHANGEPOINT_KEY}")
    print(f"k_input: {'[]' if is_empty_k(k_input) else mat2str(k_input)}")
    ...
    all_results: dict[str, Any] = {
        ...
        "dataFile": str(data_file),
        "metadata": metadata,
        "changePointKey": CHANGEPOINT_KEY,
        "timesMs": times_ms,
        "channelLabels": np.asarray(channel_labels, dtype=object),
        "nodeXY": node_xy,
        "methods": {},
    ...
    for method_name, cp_file in method_files.items():
        output_dir = output_root / method_name
        output_dir.mkdir(parents=True, exist_ok=True)
        csv_dir = output_dir / "csv"
        csv_dir.mkdir(parents=True, exist_ok=True)

        cp_ms = load_changepoints_ms(cp_file, CHANGEPOINT_KEY)
        change_points = changepoint_ms_to_frame_numbers(cp_ms, cfg, n_frames)
        intervals = boundary_frame_numbers_to_interval_specs(change_points, times_ms)

        print(f"=== {method_name} ===")
        print(f"Change-point file: {cp_file}")
        print(f"Change-point times: {mat2str(cp_ms, precision=4)} ms")
        print(f"Change points used: {mat2str(change_points)}")
        print(f"Change points 0-based: {mat2str(change_points - 1)}")

        results: dict[str, Any] = {
            "config": config_to_dict(cfg),
            "method": method_name,
            "dataFile": str(data_file),
            "changePointFile": str(cp_file),
            "changePointKey": CHANGEPOINT_KEY,
            "changePoints": change_points,
            "changePointsZeroBased": change_points - 1,
            "changePointTimesMs": times_ms[change_points - 1],
            "requestedChangePointTimesMs": cp_ms,
            "timesMs": times_ms,
            "channelLabels": np.asarray(channel_labels, dtype=object),
            "nodeXY": node_xy,
            "intervals": [interval_to_result(interval) for interval in intervals],
        ...
        for interval_idx, interval in enumerate(intervals, start=1):
            print(
                f"Preparing {interval.displayName}: frames "
                f"{interval.indices[0]}-{interval.indices[-1]} (0-based), "
                f"{interval.windowMs[0]:.1f} to {interval.windowMs[1]:.1f} ms"
            )
            ...
            fcca = run_fcca_model_selection(graphs, interval_k_range, cfg)
            key = interval.key
            final_cooccurrence = cooccurrence_from_labels(fcca["labels"])
            results[key] = {
                "displayName": interval.displayName,
                "frameRange": np.array([interval.frameNumbers[0], interval.frameNumbers[-1]], dtype=int),
                "frameRangeZeroBased": np.array([interval.indices[0], interval.indices[-1]], dtype=int),
                "frames": interval.frameNumbers.astype(int),
                "windowMs": interval.windowMs,
                "meanAdjacency": mean_adjacency,
                "labels": fcca["labels"],
                ...
                "qualityTable": fcca["qualityTableArray"],
                "qualityTableColumns": np.asarray(fcca["qualityTableColumns"], dtype=object),
                "cooccurrence": final_cooccurrence,
                "hierarchy": hierarchy_to_object_array(fcca["hierarchy"]),
            }

            write_matrix(csv_dir / f"{key}_mean_adjacency.csv", mean_adjacency)
            write_matrix(csv_dir / f"{key}_fcca_labels.csv", fcca["labels"].reshape(-1, 1), fmt="%d")
            write_matrix(csv_dir / f"{key}_cooccurrence_final_labels.csv", final_cooccurrence)
            write_quality_table(csv_dir / f"{key}_quality_by_k.csv", fcca["qualityTableRows"])

            summary_rows.append(
                {
                    "method": method_name,
                    "interval": interval.displayName,
                    "startFrame0Based": int(interval.indices[0]),
                    "endFrame0Based": int(interval.indices[-1]),
                    "startMs": float(interval.windowMs[0]),
                    "endMs": float(interval.windowMs[1]),
                    "bestK": int(fcca["bestK"]),
                    "bestSelectionScore": float(fcca["bestSelectionScore"]),
                    "bestU": float(fcca["bestQuality"]),
                    "bestQ": float(fcca["bestModularity"]),
                }
            )
        ...
        print(f"Saved {method_name} FCCA results to: {output_dir}\n")

    write_summary_table(output_root / "summary_1024Hz_ERN_by_method.csv", summary_rows)
    print(f"Saved all FCCA results to: {output_root}")
    return all_results


def resolve_data_file(base_dir: Path) -> Path:
    if DATA_FILE_OVERRIDE:
        data_file = Path(DATA_FILE_OVERRIDE)
        if not data_file.is_absolute():
            data_file = base_dir / data_file
...
def resolve_changepoint_method_files(base_dir: Path) -> dict[str, Path]:
    method_files: dict[str, Path] = {}
    for method_name, relative_file in CHANGEPOINT_METHOD_FILES.items():
        cp_file = relative_file if relative_file.is_absolute() else base_dir / relative_file
        if not cp_file.is_file():
            raise FileNotFoundError(f"Cannot find {method_name} change-point file: {cp_file}")
        method_files[method_name] = cp_file
    return method_files


def load_changepoints_ms(filename: Path, key: str) -> np.ndarray:
    loaded = np.load(filename, allow_pickle=True)
    payload = loaded.item() if loaded.shape == () else loaded
    if not isinstance(payload, dict):
        raise ValueError(f"Expected {filename} to contain a dict of change-point arrays.")
    if key not in payload:
        available = ", ".join(sorted(payload.keys()))
        raise KeyError(f"Cannot find {key} in {filename}. Available keys: {available}")
    cp_ms = np.asarray(payload[key], dtype=float).reshape(-1)
    if cp_ms.size != 4:
        raise ValueError(f"{filename} / {key} must contain exactly 4 boundaries, got {cp_ms.size}.")
    if np.any(np.diff(cp_ms) <= 0):
        raise ValueError(f"Change points must be strictly increasing: {mat2str(cp_ms, precision=4)}")
    return cp_ms


def changepoint_ms_to_frame_numbers(cp_ms: np.ndarray, cfg: Config, n_frames: int) -> np.ndarray:
    frames_0based = np.rint((cp_ms - cfg.epochStartMs) * cfg.sampleRateHz / 1000.0).astype(int)
    if np.any(frames_0based < 0) or np.any(frames_0based >= n_frames):
        raise ValueError(f"Converted frames must be between 0 and {n_frames - 1}: {mat2str(frames_0based)}")
    if np.any(np.diff(frames_0based) <= 0):
        raise ValueError(f"Converted frames must be strictly increasing: {mat2str(frames_0based)}")
    return frames_0based + 1


def boundary_frame_numbers_to_interval_specs(boundaries: np.ndarray, times_ms: np.ndarray) -> list[IntervalSpec]:
    cp = np.rint(np.asarray(boundaries, dtype=float).reshape(-1)).astype(int)
    n_frames = times_ms.size
    if cp.size != 4:
        raise ValueError("Expected exactly 4 boundary frame numbers.")
    if cp[0] < 1 or cp[-1] > n_frames or np.any(np.diff(cp) <= 0):
        raise ValueError(f"Invalid boundary frames {mat2str(cp)} for {n_frames} frames.")
...
def write_summary_table(filename: Path, rows: list[dict[str, Any]]) -> None:
    fieldnames = [
        "method",
        "interval",
        "startFrame0Based",
