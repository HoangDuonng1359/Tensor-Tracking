...
_duong_path = ROOT / "../outputs/ho_rlsl_results_Duong.npz"
HO_RLSL_RESULTS_FILE = _duong_path if _duong_path.exists() else ROOT / "../outputs/ho_rlsl_results.npz"
...
@st.cache_data(show_spinner=False)
def load_algo_change_points(cp_method, condition, n_times):
    file_path = CP_ALGO_FILES.get(cp_method)
    if not file_path or not file_path.exists():
        return []
    try:
        data = np.load(file_path, allow_pickle=True).item()
        key = _CP_KEY_MAP.get(condition)
...
def display_change_points_for_timecourse(change_point_tensor, low_rank_method, condition):
    ...
    if low_rank_method == "Ho-RLSL saved low-rank" and condition == "incorrect":
        ...
        if saved is not None:
            ...
            points = points[(points >= 0) & (points < n_times)]
            intervals, _, metadata = make_paper_like_ho_rlsl_intervals(n_times, points, target_window_ms=(-150.0, 150.0))
...
def main():
    ...
    with st.sidebar:
        ...
        low_rank_method = "Ho-RLSL saved low-rank" if condition == "incorrect" and has_saved_ho_rlsl_low_rank(str(TENSOR_FILES[condition])) else "Raw tensor"
        if condition == "incorrect" and low_rank_method == "Raw tensor":
            st.warning("Ho-RLSL saved low-rank not found. Falling back to Raw tensor.")
        ...
        saved = load_saved_ho_rlsl_change_points()
        if saved is not None and condition == "incorrect":
            points = saved["filtered_change_points"]
            points = points[(points >= 0) & (points < n_times)]
            intervals, _, metadata = make_paper_like_ho_rlsl_intervals(n_times, points, target_window_ms=(-150.0, 150.0))
            change_points = {
    ...
    tabs = st.tabs(["① Preprocessing", "② PLV Tensors", "③ Low-Rank", "④ Change Points", "⑤ FCCA Networks", "⑥ Paper Figures", "⑦ Data"])

    # ── Tab ① Preprocessing ──────────────────────────────
    with tabs[0]:
        st.subheader("EEG Preprocessing & Trial Balancing")
        report_path = prep_config.report_etl
        if report_path.exists():
            report_df = pd.read_csv(str(report_path))
            n_success = int((report_df["status"] == "SUCCESS").sum())
            m_cols = st.columns(4)
            m_cols[0].metric("Subjects", len(report_df))
            m_cols[1].metric("Successful", n_success)
            m_cols[2].metric("Channels", len(prep_config.eeg.CHANNELS))
            m_cols[3].metric("Sampling Rate", f"{prep_config.eeg.SFREQ} Hz")
            chart_df = report_df[report_df["status"] == "SUCCESS"][["sub", "correct", "incorrect"]].copy()
            if not chart_df.empty:
                chart_df = chart_df.set_index("sub")
                st.bar_chart(chart_df, height=260)
            st.dataframe(report_df, use_container_width=True, hide_index=True)
        else:
            st.warning("Run `python src/main.py` to generate preprocessing outputs.")

    # ── Tab ② PLV Tensors ────────────────────────────────
    with tabs[1]:
        st.subheader("PLV Connectivity Tensors")
        raw_tensor = tensors[condition]
        n_sub, n_ch, _, n_tw = raw_tensor.shape
        m_cols = st.columns(4)
        m_cols[0].metric("Subjects", n_sub)
        m_cols[1].metric("Channels", n_ch)
        m_cols[2].metric("Time Windows", n_tw)
        m_cols[3].metric("Tensor Size", f"{raw_tensor.nbytes / 1e6:.0f} MB")

        trace = mean_connectivity_trace(raw_tensor)
        tw_ms = frames_to_ms(n_tw)
        fig_tc, ax_tc = plt.subplots(figsize=(11, 3))
        ax_tc.plot(tw_ms, smooth_trace(trace, smooth_window), color="red", linewidth=1.5)
        ax_tc.axhline(0, color="0.65", linewidth=0.8)
        ax_tc.axvline(0, color="0.45", linewidth=0.8, linestyle="--")
        ax_tc.set_xlabel("Time (ms)")
        ax_tc.set_ylabel("z(mean PLV)")
        ax_tc.set_title(f"Mean Connectivity — {condition}")
        ax_tc.tick_params(direction="in", top=True, right=True)
        ax_tc.spines["top"].set_visible(True)
        ax_tc.spines["right"].set_visible(True)
        fig_tc.tight_layout()
        st.pyplot(fig_tc, clear_figure=True)

        st.markdown("---")
        col_s, col_t = st.columns([1, 2])
        with col_s:
            plv_sub = st.slider("Subject index", 0, n_sub - 1, 0, key="plv_sub")
            plv_tw = st.slider("Time window", 0, n_tw - 1, n_tw // 2, key="plv_tw")
            plv_ms = frames_to_ms(n_tw)[plv_tw]
            st.metric("Approx. time", f"{plv_ms:.1f} ms")
        with col_t:
            A_plv = sanitize_adjacency(raw_tensor[plv_sub, :, :, plv_tw])
            st.pyplot(
                plot_heatmap(A_plv, title=f"PLV | {condition} | sub {plv_sub + 1} | {plv_ms:.0f} ms"),
                clear_figure=True,
            )

    # ── Tab ③ Low-Rank ───────────────────────────────────
    with tabs[2]:
        st.subheader("Low-Rank Extraction")
        raw_tensor = tensors[condition]
        n_tw = raw_tensor.shape[-1]
        col_ls, col_lt = st.columns(2)
        with col_ls:
            lr_sub = st.slider("Subject", 0, raw_tensor.shape[0] - 1, 0, key="lr_sub")
        with col_lt:
            lr_tw = st.slider("Time window", 0, n_tw - 1, n_tw // 2, key="lr_tw")

        can_ho = condition == "incorrect" and has_saved_ho_rlsl_low_rank(str(TENSOR_FILES[condition]))
        if can_ho:
            denoised = load_analysis_tensor(str(TENSOR_FILES[condition]), "Ho-RLSL saved low-rank")
            raw_sl = sanitize_adjacency(raw_tensor[lr_sub, :, :, lr_tw])
            den_sl = sanitize_adjacency(denoised[lr_sub, :, :, lr_tw])
            col_a, col_b = st.columns(2)
            with col_a:
                st.pyplot(plot_heatmap(raw_sl, title="Raw PLV"), clear_figure=True)
            with col_b:
                st.pyplot(plot_heatmap(den_sl, title="HO-RLSL Low-Rank"), clear_figure=True)
            m_cols = st.columns(3)
            m_cols[0].metric("Raw ‖·‖F", f"{np.linalg.norm(raw_sl):.3f}")
            m_cols[1].metric("Low-Rank ‖·‖F", f"{np.linalg.norm(den_sl):.3f}")
            m_cols[2].metric("Sparse ‖·‖F", f"{np.linalg.norm(raw_sl - den_sl):.3f}")
        else:
            if condition != "incorrect":
                st.info("HO-RLSL low-rank is available only for the **incorrect** (ERN) condition.")
            else:
                st.warning("HO-RLSL results not found. Run the pipeline with `--low-rank-method ho-rlsl`.")
            raw_sl = sanitize_adjacency(raw_tensor[lr_sub, :, :, lr_tw])
            st.pyplot(plot_heatmap(raw_sl, title="Raw PLV"), clear_figure=True)
    ...
    with tabs[3]:
        ...
        if erp_error:
            st.warning(f"{erp_error} Falling back to z-scored mean connectivity.")

        st.markdown("---")
        cp_times = frames_to_ms(n_times)
        iv_rows = []
        for iv_name, iv_frames in intervals.items():
            iv_rows.append({
                "Interval": iv_name,
                "Start frame": int(iv_frames[0]),
                "End frame": int(iv_frames[-1]),
                "Start (ms)": f"{cp_times[iv_frames[0]]:.1f}",
                "End (ms)": f"{cp_times[iv_frames[-1]]:.1f}",
                "N frames": len(iv_frames),
            })
        ...
        if plot_path and plot_path.exists():
            ...
            st.info(f"No diagnostic plot available for {cp_method} / {condition}.")

    # ── Tab ⑤ FCCA Networks ──────────────────────────────
    with tabs[4]:
        st.subheader("FCCA Consensus Clustering")
        ...
        if matrix_source != "Tensor interval mean":
            if active_fcca_results and condition == "incorrect":
                ...
                if matrix is not None and labels is not None:
                    ...
                    st.warning("Selected interval matrix not found in saved FCCA results. Please run main.py first.")
            else:
                st.warning("FCCA results only available for the ERN (incorrect) condition using the true paper pipeline.")
        else:
            matrix = interval_mean_adjacency(selected_tensor, selected_frames)
            st.info("Displaying simple mean tensor for interval. True paper FCCA is available in the 'Matrix source' options above."),
    ...
    # ── Tab ⑥ Paper Figures ──────────────────────────────
    with tabs[5]:
        st.subheader("FCCA Paper Visualizations")
        st.info("These static figures are pre-generated by the rigorous FCCA paper pipeline (`src/fcca/cluster_ern_fcca_paper_final.py`), ensuring optimal modularity clustering without interactive lag.")
        
        vis_dir = ROOT / "../outputs/fcca_paper_visualizations"
        if vis_dir.exists():
            st.markdown("### Brain Networks")
            col1, col2 = st.columns(2)
            with col1:
                img_before = vis_dir / "brain_networks_before_fcca.png"
                if img_before.exists():
                    st.image(str(img_before), caption="Before FCCA: Mean Input Networks", use_container_width=True)
            with col2:
                img_after = vis_dir / "brain_networks_after_fcca.png"
                if img_after.exists():
                    st.image(str(img_after), caption="After FCCA: Consensus Communities", use_container_width=True)
            
            st.markdown("---")
            st.markdown("### Connectivity Matrices")
            col3, col4 = st.columns(2)
            with col3:
                mat_before = vis_dir / "connectivity_matrices_before_fcca.png"
                if mat_before.exists():
                    st.image(str(mat_before), caption="Before FCCA: Mean Adjacency Matrices", use_container_width=True)
            with col4:
                mat_after = vis_dir / "connectivity_matrices_after_fcca.png"
                if mat_after.exists():
                    st.image(str(mat_after), caption="After FCCA: Matrices sorted by Community", use_container_width=True)
        else:
            st.warning("Paper visualization figures not found. Please run the full pipeline to generate them.")
