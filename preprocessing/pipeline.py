...
def preprocess_individual_subject(subject_id: str, visualize: bool = False) -> mne.Epochs:
  """
  Preprocess EEG data following Ozdemir (2017) specifications:
  - Current Source Density (CSD) transformation applied.
  - Filtering and resampling to 128Hz.
  """
...
if __name__ == "__main__":
  ...
  print(f"Running preprocessing (Ozdemir Style) for {len(sub_ids)} subjects...")
  ...
  for i, sub in enumerate(sub_ids):
    try:
      ...
      if (i + 1) % 10 == 0:
        print(f" Progress: {i+1}/{len(sub_ids)} subjects...")
